...
function buildSeoCatalogPagesListUrl(options = {}) {
  ...
  return buildApiUrl(query ? `/seo_catalog_pages?${query}` : '/seo_catalog_pages');
}

export async function getSeoCatalogPages(options = {}) {
  try {
    const response = await fetch(buildSeoCatalogPagesListUrl(options), {
