...
// ✅ Читаем auth токен прямо из localStorage — не из React state
// Это важно: к моменту вызова fetchCartAPI React state может ещё не обновиться
...
async function fetchCartAPI(endpoint, options = {}) {
  ...
  // ✅ Автоматически добавляем Authorization если пользователь авторизован
...
export async function getCart({ promo_code } = {}) {
  ...
  if (!token && !getAuthToken()) return { cart: null };
  try {
