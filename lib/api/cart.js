...
// Корзина: если авторизован — Bearer, иначе cart_token
...
export async function getCart({ promo_code } = {}) {
  ...
  // Если не авторизован и нет cart_token — корзина пуста
  if (!authToken && !token) return { cart: null };

  try {
    const params = new URLSearchParams();
    // cart_token передаём только для гостя — авторизованный определяется по Bearer
    if (!authToken && token) params.set('cart_token', token);
  ...
  try {
    const response = await fetchCartAPI('/cart_items', {
      ...
      body: JSON.stringify({
        sku,
        quantity,
        ...(!authToken && token ? { cart_token: token } : {}),
      }),
    });
    if (response.cart?.token) setCartToken(response.cart.token);
    return response;
