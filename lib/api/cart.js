...
export async function addToCart(sku, quantity = 1) {
    ...
    try {
        // КОСТЫЛЬ: бэкенд добавляет +1, поэтому отправляем quantity - 1
        ...
        // Сохраняем токен (если это первое добавление)
        if (response.cart?.token) {
            setCartToken(response.cart.token);
        }
...
export async function updateCartItemQuantity(sku, newQuantity) {
    ...
    try {
        ...
        // ШАГ 2: Добавить с новым количеством (КОСТЫЛЬ: -1, т.к. бэкенд добавляет +1)
        if (newQuantity > 0) {
            const response = await fetchCartAPI('/cart_items', {
                method: 'POST',
                body: JSON.stringify({
...
export async function checkout(orderData) {
    try {
        ...
        // Очищаем токен корзины после успешного заказа
        removeCartToken();
