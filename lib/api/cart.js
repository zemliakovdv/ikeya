...
// Корзина работает только через cart_token — Bearer не передаём в cart API
async function fetchCartAPI(endpoint, options = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const err = new Error(payload.message || payload.error || `API Error: ${response.status}`);
      err.status = response.status;
      err.payload = payload; // пробрасываем весь payload — нужен для passport_verification_required
      throw err;
    }
...
export async function getCart({ promo_code } = {}) {
  ...
  if (!token && !authToken) return { cart: null };
  try {
    const params = new URLSearchParams();
    if (token) params.set('cart_token', token);
    if (promo_code) params.set('promo_code', promo_code);
    const headers = {};
    if (authToken) headers['Authorization'] = `Bearer ${authToken}`;
    const response = await fetchCartAPI(`/cart?${params.toString()}`, { headers });
...
export async function checkout(orderData, authToken) {
  try {
    ...
    const response = await fetchCartAPI('/checkout', {
      ...
      body: JSON.stringify({ ...orderData, ...(cartToken ? { cart_token: cartToken } : {}) }),
    });
    removeCartToken();
    return response;
  } catch (error) {
