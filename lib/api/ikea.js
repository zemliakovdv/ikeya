...
export async function getCachedCategoriesTree() {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`API Error: ${response.status}`);
    const data = await response.json();
    const tree = (data.data || []).map((cat) => normalizeTreeNode(cat));

    _categoriesTreeCache = tree;
    _categoriesTreeCacheTime = now;
    return tree;
  } catch (error) {
...
let _newArrivalsCacheTime = 0;

export async function getAllNewArrivals() {
  const now = Date.now();
  if (_newArrivalsCache && (now - _newArrivalsCacheTime) < CACHE_TTL) {
    return _newArrivalsCache;
  }
  try {
    const response = await fetchAPI('/products/new_arrivals?per_page=50');
    const products = response.data || [];
    _newArrivalsCache = products;
    _newArrivalsCacheTime = now;
    return products;
  } catch (error) {
    console.error('Ошибка загрузки новинок:', error.message);
    return [];
  }
}

let _recommendedCache = null;
let _recommendedCacheTime = 0;

export async function getAllRecommended() {
  const now = Date.now();
  if (_recommendedCache && (now - _recommendedCacheTime) < CACHE_TTL) {
    return _recommendedCache;
  }
  try {
    const response = await fetchAPI('/products/recommended?per_page=50');
    const products = response.data || [];
    _recommendedCache = products;
    _recommendedCacheTime = now;
    return products;
  } catch (error) {
    console.error('Ошибка загрузки рекомендованных:', error.message);
    return [];
  }
}
