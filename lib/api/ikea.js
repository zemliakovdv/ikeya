...
async function fetchAPI(endpoint, options = {}) {
  ...
  try {
    const response = await fetch(url, {
      ...options,
      cache: 'no-store' // отключаем fetch-кеш Next.js (он не тянет >2MB)
    });
...
// ✅ unstable_cache работает на уровне файловой системы — держит кеш между запросами
const _fetchAllCategories = unstable_cache(
  async () => {
    console.log('📦 Fetching categories from API...');
    const response = await fetch(`${API_BASE_URL}/categories?per_page=500`, {
      cache: 'no-store'
    });
    if (!response.ok) throw new Error(`API Error: ${response.status}`);
