...
export const IMAGES_BASE_URL = 'https://test.ikeya.by';
...
export function resolveImageUrl(path) {
  ...
  // Убираем все ведущие слеши — добавим ровно один
  const clean = path.replace(/^\/+/, '');
  return `${IMAGES_BASE_URL}/${clean}`;
}
...
export async function getCategoryProducts(
  ...
  try {
    ...
    const minPrice = extraParams.min_price ? parseFloat(extraParams.min_price) : null;
    const maxPrice = extraParams.max_price ? parseFloat(extraParams.max_price) : null;
