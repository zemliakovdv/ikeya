// lib/utils/categoryHelpers.js

/**
 * Поиск категории по id (в корне объекта)
 */
export function findCategoryByIkeaId(categories, ikeaId) {
  return (categories || []).find((cat) => cat?.id === ikeaId);
}

/**
 * Поиск категории по slug (безопасно)
 */
export function findCategoryBySlug(categories, slug) {
  if (!slug) return undefined;
  return (categories || []).find((cat) => cat?.attributes?.slug === slug);
}

/**
 * Строим полный URL категории из цепочки slug'ов
 */
export function buildCategoryUrl(categoryChain) {
  const slugs = (categoryChain || [])
    .map((cat) => cat?.attributes?.slug)
    .filter(Boolean);

  return slugs.length ? `/catalog/${slugs.join('/')}` : '/catalog';
}

/**
 * Построение цепочки категорий от корня до текущей
 * parent_ids: массив id родителей от корня к текущей
 */
export function buildCategoryChain(categories, currentCategory) {
  if (!currentCategory) return [];

  const chain = [currentCategory];
  const parentIds = currentCategory.attributes?.parent_ids || [];

  for (let i = 0; i < parentIds.length; i++) {
    const parent = findCategoryByIkeaId(categories, parentIds[i]);
    if (parent && !chain.find((c) => c.id === parent.id)) {
      chain.unshift(parent);
    }
  }

  return chain;
}

/**
 * Получение дочерних категорий (по последнему parent_id)
 */
export function getChildCategories(categories, parentIkeaId) {
  return (categories || []).filter((cat) => {
    const parentIds = cat?.attributes?.parent_ids || [];
    return parentIds[parentIds.length - 1] === parentIkeaId;
  });
}

/**
 * Breadcrumbs по slug-цепочке
 */
export function buildBreadcrumbs(categoryChain) {
  const breadcrumbs = [
    { name: 'Главная', href: '/' },
    { name: 'Каталог', href: '/catalog' },
  ];

  let currentPath = '/catalog';

  (categoryChain || []).forEach((category) => {
    const slug = category?.attributes?.slug;
    const name = category?.attributes?.translated_name;
    if (!slug || !name) return;

    currentPath += `/${slug}`;

    breadcrumbs.push({
      name,
      href: currentPath,
    });
  });

  return breadcrumbs;
}

/**
 * ✅ ВАЖНО: разворачиваем /categories/tree в плоский массив ВСЕХ нод
 * В дереве обычно есть children / subcategories (название может отличаться)
 */
export function flattenCategoriesTree(treeData) {
  const result = [];

  const walk = (nodeOrList) => {
    if (!nodeOrList) return;

    if (Array.isArray(nodeOrList)) {
      nodeOrList.forEach(walk);
      return;
    }

    const node = nodeOrList;

    if (node?.id && node?.attributes) {
      result.push({
        id: node.id,
        type: node.type,
        attributes: node.attributes,
        relationships: node.relationships,
      });
    }

    // поддержим разные ключи детей (часто: children)
    const children =
      node.children ||
      node.subcategories ||
      node.items ||
      node.nodes ||
      [];

    if (Array.isArray(children) && children.length > 0) {
      children.forEach(walk);
    }
  };

  walk(treeData);
  return result;
}

/**
 * Поиск узла в оригинальном дереве по цепочке slug'ов.
 * Возвращает объект:
 * {
 *   node,        — найденный узел (с children из дерева)
 *   ancestors,   — массив всех предков от корня до (не включая) текущей
 *                  например для уровня 3: [корневая, родитель]
 *   siblings,    — дети непосредственного родителя (категории того же уровня)
 * }
 * slugChain — массив slug'ов от корня до текущей категории,
 * например ['vannye-komnaty', 'mebel-enhet', 'tumby']
 *
 * Если slugChain содержит только один элемент и он не найден среди корневых —
 * выполняется рекурсивный поиск по всему дереву с восстановлением ancestors.
 */
export function findNodeInTree(treeData, slugChain) {
  if (!treeData || !slugChain || slugChain.length === 0) {
    return { node: null, ancestors: [], siblings: [] };
  }

  const roots = Array.isArray(treeData) ? treeData : [];

  // ── Обычный поиск по цепочке slug'ов ────────────────────────────────────
  let currentList = roots;
  const ancestors = [];
  let node = null;

  for (let i = 0; i < slugChain.length; i++) {
    const slug = slugChain[i];
    const found = currentList.find((n) => n?.attributes?.slug === slug);

    if (!found) {
      // slug не найден на текущем уровне — пробуем рекурсивный поиск
      break;
    }

    if (i < slugChain.length - 1) {
      ancestors.push(found);
    }

    node = found;
    currentList = found.children || [];
  }

  // Если нашли узел обычным способом — возвращаем результат
  if (node && node.attributes?.slug === slugChain[slugChain.length - 1]) {
    const directParent = ancestors.length > 0 ? ancestors[ancestors.length - 1] : null;
    const siblings = directParent ? (directParent.children || []) : [];
    return { node, ancestors, siblings };
  }

  // ── Рекурсивный поиск по всему дереву ───────────────────────────────────
  // Используется когда slug не является корневым (например ссылка из хедера
  // ведёт сразу на глубокую категорию без полного пути)
  const targetSlug = slugChain[slugChain.length - 1];

  function searchInTree(nodeList, parentChain) {
    for (const n of nodeList) {
      if (!n?.attributes?.slug) continue;

      if (n.attributes.slug === targetSlug) {
        return { node: n, ancestors: parentChain };
      }

      const children = n.children || [];
      if (children.length > 0) {
        const result = searchInTree(children, [...parentChain, n]);
        if (result) return result;
      }
    }
    return null;
  }

  const found = searchInTree(roots, []);

  if (found) {
    const directParent = found.ancestors.length > 0
      ? found.ancestors[found.ancestors.length - 1]
      : null;
    const siblings = directParent ? (directParent.children || []) : [];
    return { node: found.node, ancestors: found.ancestors, siblings };
  }

  return { node: null, ancestors: [], siblings: [] };
}

/**
 * Рекурсивный сбор всех дочерних категорий (если нужен)
 */
export function getAllChildCategoryIds(categories, parentIkeaId) {
  const result = [parentIkeaId];

  function collectChildren(ikeaId) {
    (categories || [])
      .filter((cat) => (cat?.attributes?.parent_ids || []).includes(ikeaId))
      .forEach((child) => {
        if (child?.id && !result.includes(child.id)) {
          result.push(child.id);
          collectChildren(child.id);
        }
      });
  }

  collectChildren(parentIkeaId);
  return result;
}
/**
 * Строим breadcrumbs из дерева категорий по цепочке slug'ов.
 * Работает как с полной slugChain (['root', 'child', 'grandchild']),
 * так и с одним slug — в этом случае рекурсивно ищет узел в дереве
 * и восстанавливает полный путь предков.
 */
export function buildBreadcrumbsFromTree(treeData, slugChain) {
  const breadcrumbs = [
    { name: 'Главная', href: '/' },
    { name: 'Каталог', href: '/catalog' },
  ];

  const { node, ancestors } = findNodeInTree(treeData, slugChain);

  if (!node) return breadcrumbs;

  // ancestors уже содержит полный путь от корня до текущей
  let currentPath = '/catalog';

  ancestors.forEach((ancestor) => {
    const slug = ancestor?.attributes?.slug;
    const name = ancestor?.attributes?.translated_name || ancestor?.attributes?.name;
    if (!slug || !name) return;
    currentPath += `/${slug}`;
    breadcrumbs.push({ name, href: currentPath });
  });

  // Текущий узел — последний, без ссылки (или со ссылкой на себя)
  const currentSlug = node?.attributes?.slug;
  const currentName = node?.attributes?.translated_name || node?.attributes?.name;
  if (currentSlug && currentName) {
    currentPath += `/${currentSlug}`;
    breadcrumbs.push({ name: currentName, href: currentPath });
  }

  return breadcrumbs;
}