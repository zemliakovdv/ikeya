...
function resolveImage(path) {
  if (!path) return '/assets/img/profile/active_1.png';
  if (path.startsWith('http')) return path;
  return `${IMAGES_BASE}/${path.replace(/^\/+/, '')}`;
}
...
const STATUS_MAP = {
  'assembly': { label: 'В обработке', cls: 'badge-assembly' },
  'awaiting': { label: 'Ждёт оплаты', cls: 'badge-awaiting' },
  'transit': { label: 'Оформлен', cls: 'badge-available' },
  'available-warehouse': { label: 'Получен на склад', cls: 'badge-available' },
  'assembly-process': { label: 'Подготовка и сборка заказа', cls: 'badge-available' },
  'customs-poland': { label: 'Таможня Польша', cls: 'badge-available' },
  'customs-belarus': { label: 'Таможня Беларусь', cls: 'badge-available' },
  'in-transit-pvz': { label: 'В доставке ПВЗ', cls: 'badge-available' },
  'arrived-pvz': { label: 'Прибыл в ПВЗ', cls: 'badge-ready' },
};
...
export default function ProfileDashboard() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        const [ordersResp, purchasesResp, reviewsResp] = await Promise.allSettled([
          getOrders({ per_page: 50 }),
          fetchWithAuth('/account/purchases?per_page=1'),
          fetchWithAuth('/account/reviews?per_page=1'),
        ]);

        if (ordersResp.status === 'fulfilled') {
          const all = ordersResp.value.data || [];
          const active = all.filter(o => isActiveOrder(o.attributes.status));

          // Догружаем детали заказа чтобы получить картинку первого товара
          const withDetails = await Promise.allSettled(
            active.slice(0, 6).map(async (order) => {
              try {
                const detail = await fetchWithAuth(`/account/orders/${order.attributes.id}`);
                const firstItem = detail.included?.[0];
                return { ...order, imageUrl: firstItem?.attributes?.image_url || null };
              } catch {
          ...
          setActiveOrders(
            withDetails.filter(r => r.status === 'fulfilled').map(r => r.value)
          );
        }
        ...
        if (reviewsResp.status === 'fulfilled') {
          setReviewsTotal(reviewsResp.value.meta?.total ?? 0);
        }
      } catch (e) {
      ...
      <div className="summary">
        <div className="summary-card" onClick={() => router.push('/profile/favorite')} style={{ cursor: 'pointer' }}>
          <div className="summary-title">
            Избранное
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M12 20.61C11.34 20.61 10.67 20.4 10.1 19.97C7.66 18.15 2 13.43 2 8.92C2 5.82 4.35 3.39 7.35 3.39C9.01 3.39 10.43 4.01 12 5.45C13.57 4.01 14.99 3.39 16.65 3.39C19.65 3.39 22 5.82 22 8.92C22 13.42 16.33 18.14 13.9 19.97C13.33 20.39 12.67 20.61 12 20.61Z" fill="#E0E0E0" /></svg>
          </div>
      ...
      <div className="block">
        ...
        <div className="orders orders-grid">
          ...
          {!loading && activeOrders.map((order) => {
            ...
            const status = STATUS_MAP[a.status] || { label: a.status, cls: 'badge-assembly' };
            ...
            return (
              ...
              >
                ...
                <div className="order-info">
                  <div className="order-title">Заказ № {a.id} от {date}</div>
...
onClick={openJivoChat}
          ...
          <button className="help-link-card" type="button" onClick={() => router.push('/help')}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
