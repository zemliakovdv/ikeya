...
function getItemSku(item, product) {
  return (
    item?.sku ||
    item?.product_sku ||
    product?.sku ||
...
function buildProductPath(product, sku) {
  if (!sku) return '#';
...
function getProductImages(product) {
  const rawImages = product?.local_images || product?.images?.local_images || product?.images;
...
function formatPrice(price) {
  ...
  const [whole, cents] = safePrice.toFixed(2).split('.');

  return {
    whole: Number(whole).toLocaleString('ru-RU'),
    cents,
  };
}
...
export default function CartItem({
  ...
  const imageUrl = resolveImageUrl(images[0]) || '/assets/img/no-image.jpg';
  const productUrl = buildProductPath(product, sku);

  const productPrice = parsePrice(product.price_byn);
  const pricingNew = parsePrice(pricing.unit_price_new_byn);
  const pricingOld = parsePrice(pricing.unit_price_old_byn);
  const discount = parsePrice(pricing.unit_discount_byn);
  ...
  const oldPriceFormatted = formatPrice(oldPrice * quantity);
  const newPriceFormatted = formatPrice(newPrice * quantity);
    ...
    <div className={`cart-item ${isUnavailable ? 'cart-item--unavailable' : 'available'}`}>
      <div className="cart-item__select">
        <input
          ...
          checked={checked}
          onChange={(e) => onCheckChange?.(sku, e.target.checked)}
          disabled={loading || isUnavailable || !sku}
        />
      </div>
      ...
      <div className="cart-item__image">
        <Link href={productUrl}>
          <img
            src={imageUrl}
            alt={product.small_desc_name || product.name || product.name_ru || 'Товар'}
      ...
      <div className="cart-item__info">
        <div className="cart-item__title-row">
          <Link href={productUrl} className="cart-item__name">
            {product.name_ru || product.name || 'Товар'}
          </Link>

          {(product.small_desc_name || product.name) && (
